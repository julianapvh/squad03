...
while True:
 ...
 if opcoes == '1':
  ...
  if opcoes_entrega == '1':
    print('Dentro de “Minha conta” clique em “Meus pedidos” e procure pela sua comprinha. Elas estão organizadas pelo número de identificação. Para ver mais detalhes clique em VER DETALHES.\nO código de rastreio deve aparecer logo abaixo do resumo da sua compra. Mas lembre-se que ele aparece de formas diferentes dependendo da forma de entrega que você selecionou.\nCom o código de rastreio em mãos, basta você localizar a sua comprinha no site dos Correios.')
   ...
   print('Os Correios não permitem que a gente altere nenhuma informação após o despacho da sua comprinha. Por isso, vamos ter que aguardar a sua comprinha retornar para a nossa loja para um novo despacho..')
 ...
 if opcoes == '2':
  ...
  opcoes_pedidos_pagamentos = (input('Escolha uma opção: \n1 - Qual o prazo de aprovação do pedido?  \n2 - Quais são as formas de pagamentos disponíveis?  \n3 - Como faço pra conseguir a 2° via do boleto? \n'))
 
 
    
  
  if opcoes_pedidos_pagamentos == '1':
    print('Isso vai depender da sua forma de pagamento. Se for cartão de crédito ou pix a compra será aprovada na mesma hora, já se for via boleto bancário a aprovação será aprovada entre 1 e 2 dia úteis')
          
    
  elif opcoes_pedidos_pagamentos == '2':
   print('Você pode pagar suas compras nos cartões de débito e crédito ou via pix.')
   

   
  elif opcoes_pedidos_pagamentos == '3':
   print('Para regerar seu boleto na loja do SQUAD 03 é preciso acessar o site da loja e clicar no menu superior que levará aos seus pedidos. Aí você entra com seu login e a respectiva senha do cadastro para ter acesso às opções da área do cliente.\nUma vez logado no sistema, vá até o menu de exibição de detalhes e escolha a opção para imprimir a 2ª via do boleto. Prontinho: já regerou sua 2° via!')
 ...
 if opcoes == '4':
   ...
   if opcoes_produtos == '1':
    print('Você pode entrar em contato direto com a nossa central de atendimento pelo SAC 2.0 X 3.0 X 4.0')
          
    
   elif opcoes_produtos == '2':
    print('Você pode adicionar o produto a sua lista de desejos e quando o item estiver disponivel você receberá um e-mail informando que o item está disponivel para venda em nosso site.')
   

   
   elif opcoes_produtos == '3':
    print('Sim, nós contamos com diversas filiais espalhadas pelo Brasil. Você pode escolher a mais próxima de você para retirar seus produtos.')
 ...
 if opcoes == '5':
   ...
   if opcoes_cadastro == '1':
    print('1 - Visite o site squad03.com No canto superior direito da tela, você verá um menu marcado como “Minha Conta.” Quando você passar o mouse sobre ele, um menu suspenso irá surgir. Clique no link "Comece Aqui/Start Here" logo abaixo do botão azul "Inscreva-se/Sign In".\n2 - Insira suas informações pessoais. O formulário de registro vai pedir seu nome e endereço de e-mail, além de solicitar que você escolha uma senha.Note que você também pode digitar um número de telefone. Isso não é obrigatório, mas fornece uma proteção melhor para a sua conta. O SQUAD 03 não vai te ligar, então não se preocupe com isso – esse telefone é usado apenas para fins de segurança.\n3 - Crie sua conta. Depois de preencher todas as informações necessárias, clique em “Criar Conta/Create Account.” Você será redirecionado imediatamente para a página de boas-vindas do SQUAD 03. Parabéns! Agora você tem uma conta oficial no site SQUAD 03.')
